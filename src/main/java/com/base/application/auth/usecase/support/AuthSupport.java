package com.base.application.auth.usecase.support;
...
import com.base.application.auth.port.RefreshTokenStorePort;
import com.base.application.auth.usecase.result.AuthSession;
import com.base.application.menu.query.UserMenuQueryService;
import com.base.application.menu.query.UserMenuQueryService.UserMenuAccessResult;
...
import com.base.application.auth.port.RefreshTokenStorePort.StoredRefreshToken;
...
public class AuthSupport {
    ...
    /**
     * 사용자 엔터티와 메뉴/권한 정보를 조립해 세션 스냅샷을 만든다.
     */
    public AuthSession buildSession(Long userId, Collection<? extends GrantedAuthority> authorities) {
        User user = userPersistencePort.findById(userId)
                .orElseThrow(() -> new NotFoundException("User not found"));

        UserResult userResult = userResultAssembler.toResult(user);
        UserMenuAccessResult menuAccess = userMenuQueryService.getAccessibleMenus(userId);

        List<String> permissions = resolvePermissions(userId, authorities);

        List<MenuTreeResult> menuTree = menuAccess.menuTree();
        List<MenuResult> flatMenus = menuAccess.flatMenus();

        return new AuthSession(userResult, menuTree, flatMenus, permissions);
    }
