package com.base.application.role.usecase.update;
...
public class UpdateRoleUseCaseImpl implements UpdateRoleUseCase {
    ...
    @Override
    public RoleResult handle(Long id, UpdateRoleCommand command) {
        Role existing = rolePersistencePort.findById(id)
                .orElseThrow(() -> new NotFoundException("Role not found"));

        if (!existing.getRoleName().equals(command.roleName())
                && rolePersistencePort.existsByRoleName(command.roleName())) {
            throw new ConflictException("Role name already exists: " + command.roleName());
        }

        existing.setRoleName(command.roleName());
        if (command.useYn() != null) {
            existing.setUseYn(command.useYn());
        }
        Role saved = rolePersistencePort.save(existing);
